...
class vn {
  constructor(t, o, n, { pixelBudget: r = 46e5 } = {}) {
    this.renderer = t, this.scene = o, this.camera = n, this.pixelBudget = r, this.size = new a.Vector2(1, 1);
    const s = { type: a.HalfFloatType, minFilter: a.LinearFilter, magFilter: a.LinearFilter, depthBuffer: true, stencilBuffer: false, colorSpace: a.NoColorSpace };
    this.rtScene = new a.WebGLRenderTarget(2, 2, s), this.rtScene.depthTexture = new a.DepthTexture(2, 2), this.rtScene.depthTexture.format = a.DepthFormat, this.rtScene.depthTexture.type = a.UnsignedIntType, this.rtScene.depthTexture.minFilter = a.NearestFilter, this.rtScene.depthTexture.magFilter = a.NearestFilter, this.rtA = new a.WebGLRenderTarget(2, 2, { ...s, depthBuffer: false }), this.rtB =
    new a.WebGLRenderTarget(2, 2, { ...s, type: a.UnsignedByteType, depthBuffer: false });
  ...
  setSize(t, o) {
    ...
    const s = Math.max(2, Math.floor(t * r)), c = Math.max(2, Math.floor(o * r));
    this.size.set(s, c), this.renderer.setPixelRatio(1), this.renderer.setSize(t, o, true), this.rtScene.setSize(s, c), this.rtA.setSize(s, c), this.rtB.setSize(s, c);
    const i = new a.Vector2(1 / s, 1 / c);
...
function kn(e = 3) {
  ...
  for (let s = 0; s < o.length; s++) n[s * 4 + 0] = o[s], n[s * 4 + 1] = o[s], n[s * 4 + 2] = o[s], n[s * 4 + 3] = 255;
  const r = new a.DataTexture(n, o.length, 1, a.RGBAFormat);
  return r.minFilter = a.NearestFilter, r.magFilter = a.NearestFilter, r.generateMipmaps = false, r.needsUpdate = true, Be.set(t, r), r;
}
...
function Gn(e, t) {
  ...
  e.userData.shadowTint = o, e.onBeforeCompile = (r) => {
    r.uniforms.uShadowTint = o, r.fragmentShader = r.fragmentShader.replace(`#include <${Ho}>`, jo);
  };
...
function b(e = {}) {
  const { color: t = 16777215, bands: o = 3, tint: n = 7102348, flat: r = true, map: s = null, emissive: c = null, emissiveIntensity: i = 1, transparent: l = false, opacity: f = 1, side: u = a.FrontSide, alphaTest: d = 0, depthWrite: h = null, fog: p = true, alphaMap: m = null, vertexColors: w = false, cache: M = true } = e, G = M && !s && !m ? [t, o, n, r, c, i, l, f, u, d, h, p, w].join("|") : null;
  if (G && Ee.has(G)) return Ee.get(G);
  const A = new a.MeshToonMaterial({ color: t, gradientMap: kn(o), flatShading: r, map: s, alphaMap: m, transparent: l, opacity: f, side: u, alphaTest: d, fog: p, vertexColors: w, emissive: c === null ? 0 : c, emissiveIntensity: i });
  return h !== null && (A.depthWrite = h), Gn(A, n), G && Ee.set(G, A), A;
}
const Ie = /* @__PURE__ */ new Map();
function I(e = {}) {
  const { color: t = 16777215, map: o = null, transparent: n = false, opacity: r = 1, side: s = a.FrontSide, alphaTest: c = 0, depthWrite: i = null, fog: l = true, cache: f = true, toneMapped: u = true } = e, d = f && !o ? [t, n, r, s, c, i, l, u].join("|") : null;
  if (d && Ie.has(d)) return Ie.get(d);
  const h = new a.MeshBasicMaterial({ color: t, map: o, transparent: n, opacity: r, side: s, alphaTest: c, fog: l, toneMapped: u });
...
function Gt(e, t, o, { srgb: n = true, repeat: r = null, aniso: s = 4 } = {}) {
  const c = document.createElement("canvas");
  c.width = e, c.height = t;
  const i = c.getContext("2d");
  i.imageSmoothingEnabled = true, o(i, e, t);
  const l = new a.CanvasTexture(c);
  return n && (l.colorSpace = a.SRGBColorSpace), l.anisotropy = s, r && (l.wrapS = l.wrapT = a.RepeatWrapping, l.repeat.set(r[0], r[1])), l.needsUpdate = true, l;
}
function Tt(e, t) {
  return De.has(e) || De.set(e, t()), De.get(e);
}
const Vt = (e) => "#" + e.toString(16).padStart(6, "0");
function Tn(e, t, o, n, r = Te, s = "bold") {
  let c = n;
  do {
    if (e.font = `${s} ${c}px ${r}`, e.measureText(t).width <= o) break;
    c -= 2;
  } while (c > 6);
  return c;
}
function $t(e, t, o, n, r, s, c, i = "bold", l = 0) {
  const f = Tn(e, t, r, s, Te, i);
...
function An(e, t, o, n, r, s, c) {
  e.font = `bold ${s}px ${Te}`, e.fillStyle = c, e.textAlign = "center", e.textBaseline = "middle", [...t].forEach((i, l) => e.fillText(i, o, n + l * r));
}
const Cn = () => Tt("crossingSign", () => Gt(512, 256, (e, t, o) => {
  e.fillStyle = "#fbf8f2", e.fillRect(0, 0, t, o), e.strokeStyle = Vt(g.black), e.lineWidth = 12, e.strokeRect(6, 6, t - 12, o - 12), $t(e, "LEVEL CROSSING", t / 2, o * 0.36, t - 60, 84, Vt(g.redDeep), "bold", 6), $t(e, "STOP \xB7 LOOK \xB7 LISTEN", t / 2, o * 0.74, t - 80, 42, Vt(g.black), "bold", 2);
})), Rn = () => Tt("stationSign", () => Gt(768, 192, (e, t, o) => {
  e.fillStyle = "#fbfaf6", e.fillRect(0, 0, t, o), e.fillStyle = Vt(g.teal), e.fillRect(0, o - 22, t, 22), $t(e, "\u0C35\u0C3F\u0C26\u0C4D\u0C2F\u0C3E\u0C28\u0C17\u0C30\u0C4D", t / 2, o * 0.42, t * 0.7, 92, "#2b3346", "bold", 12), e.font = `600 34px ${Te}`, e.fillStyle = "#8a8fa0", e.textAlign = "center", e.fillText("VIDYANAGAR", t / 2, o * 0.82);
})), Ko = (e = 0) => Tt("warnPlate" + e, () => Gt(256, 512, (t, o, n) => {
  const r = [{ bg: g.yellow, fg: g.black, t: "CCTV" }, { bg: g.red, fg: 16644336, t: "DANGER" }, { bg: 16644336, fg: g.blueDeep, t: "SLOW" }, { bg: 16644336, fg: g.redDeep, t: "11 KV" }], s = r[e % r.length];
  t.fillStyle = Vt(s.bg), t.fillRect(0, 0, o, n), t.fillStyle = Vt(s.fg), t.fillRect(12, 12, o - 24, 6), t.fillRect(12, n - 18, o - 24, 6), An(t, s.t, o / 2, 90, 88, 74, Vt(s.fg));
  ...
  if (t.fillStyle = Vt(g.tactile), t.fillRect(0, 0, o, n), t.fillStyle = "#d9a91f", e) for (let r = 0; r < 4; r++) t.fillRect(10, 12 + r * 30, o - 20, 16);
  else for (let r = 0; r < 4; r++) for (let s = 0; s < 4; s++) t.beginPath(), t.arc(20 + s * 29, 20 + r * 29, 9, 0, Math.PI * 2), t.fill();
  ...
  e.fillStyle = "#ffffff";
  for (const [r, s, c] of n) e.beginPath(), e.ellipse(r * t, s * o, c * t * 0.55, c * o * 1.1, 0, 0, Math.PI * 2), e.fill();
...
function pt(e) {
  let t = e.map(({ geometry: s, matrix: c }) => {
    const i = s.clone();
    return c && i.applyMatrix4(c), i;
  });
  const o = t.filter((s) => s.index).length;
  o > 0 && o < t.length && (t = t.map((s) => {
    if (!s.index) return s;
    const c = s.toNonIndexed();
    return s.dispose(), c;
  }));
  const n = t.reduce((s, c) => s.filter((i) => c.attributes[i] !== void 0), Object.keys(t[0].attributes));
  for (const s of t) for (const c of Object.keys(s.attributes)) n.includes(c) || s.deleteAttribute(c);
  const r = bn(t, false);
  return t.forEach((s) => s.dispose()), r;
}
const Ln = new a.Matrix4(), no = new a.Quaternion(), ao = new a.Euler(), so = new a.Vector3(), ro = new a.Vector3();
function T(e = 0, t = 0, o = 0, n = 0, r = 0, s = 0, c = 1, i = 1, l = 1) {
  return so.set(e, t, o), ao.set(n, r, s), no.setFromEuler(ao), ro.set(c, i, l), Ln.clone().compose(so, no, ro);
}
function D(e, t, o, n, r = 0, s = 0, c = 0) {
  const i = new a.Mesh(new a.BoxGeometry(e, t, o), n);
  return i.position.set(r, s, c), i;
}
function Jt(e, t, o, n, r, s = 0, c = 0, i = 0) {
  const l = new a.Mesh(new a.CylinderGeometry(e, t, o, n), r);
  return l.position.set(s, c, i), l;
}
...
function _n(e, t, o, n = 14) {
  const r = [];
  for (let s = 0; s <= n; s++) {
    const c = s / n, i = new a.Vector3().lerpVectors(e, t, c);
    i.y -= Math.sin(Math.PI * c) * o, r.push(i);
  }
  return new a.CatmullRomCurve3(r);
}
function Nn(e, t = 500) {
  ...
  const s = Dn(), c = nt(7781), i = new a.Group(), l = I({ color: g.cloud, map: s, transparent: true, opacity: 0.62, depthWrite: false, fog: false, cache: false }), f = I({ color: g.cloudShade, map: s, transparent: true, opacity: 0.34, depthWrite: false, fog: false, cache: false });
...
const F = 3.15, K = 1.55, Lt = 0.135, On = 0.015, _t = -80, Nt = 80, bt = 2.2, ke = 2.95, It = 3.35;
function U(e) {
  let t = 0;
...
function Vn(e, t) {
  ...
  return Math.abs(t) < It ? false : o > F - 0.02 && o < F + K;
}
function io(e, t) {
  return et() + (Vn(e, t) ? Lt : 0);
}
function Xt({ z0: e, z1: t, step: o = 1.2, a: n, b: r, uv: s = [1, 1], flip: c = false }) {
  const i = Math.max(2, Math.round(Math.abs(t - e) / o) + 1), l = [], f = [], u = [];
  for (let h = 0; h < i; h++) {
    const p = h / (i - 1), m = e + (t - e) * p, w = n(m), M = r(m);
    l.push(w.x, w.y, m, M.x, M.y, m), f.push(0, p * s[1], s[0], p * s[1]);
  }
  for (let h = 0; h < i - 1; h++) {
    const p = h * 2;
...
function Hn(e, t, o = new a.Vector3()) {
  const n = e / Y, r = t / Y, s = Math.cos(r);
  return o.set(Math.sin(n) * s, Math.cos(n) * s, Math.sin(r));
}
function Yt(e, t, o, n = new a.Vector3()) {
  return Hn(e, o, n).multiplyScalar(Y + t).add(ot), n;
}
function te(e, t, o = new a.Vector3(), n = new a.Vector3(), r = new a.Vector3()) {
  const s = e / Y, c = t / Y, i = Math.sin(s), l = Math.cos(s), f = Math.sin(c), u = Math.cos(c);
...
function Un(e, t) {
  ...
  const n = t * t;
  let s = Object.keys(o.attributes).map((u) => ({ name: u, size: o.attributes[u].itemSize, src: o.attributes[u].array })), c = o.attributes.position.count;
  ...
  for (let u = 0; u < 12; u++) {
    const d = s.find((w) => w.name === "position").src;
    let h = 0;
    const p = s.map((w) => ({ name: w.name, size: w.size, dst: [] })), m = [];
    for (let w = 0; w < c; w += 3) {
      let M = -1, G = 0;
      for (let y = 0; y < 3; y++) {
      ...
      if (M <= n) {
        for (const y of p) {
          const k = s.find((P) => P.name === y.name).src;
      ...
      for (const y of p) {
        const k = s.find((z) => z.name === y.name).src, P = [];
    ...
    if (s = p.map((w) => ({ name: w.name, size: w.size, src: Float32Array.from(w.dst) })), l = Int32Array.from(m), c = s.find((w) => w.name === "position").src.length / 3, !h) break;
  }
  const f = new a.BufferGeometry();
  for (const u of s) f.setAttribute(u.name, new a.BufferAttribute(u.src, u.size));
  if (i) {
    let u = 0;
...
function jn(e, t = 3) {
  const o = Un(e, t), n = o.attributes.position, r = new a.Vector3();
  for (let s = 0; s < n.count; s++) Yt(n.getX(s), n.getY(s), n.getZ(s), r), n.setXYZ(s, r.x, r.y, r.z);
...
function Kn(e, { maxEdge: t = 3 } = {}) {
  ...
  const s = { wrapped: 0, instanced: 0, rigid: n.length, tris: 0 };
  for (const { obj: i, world: l } of r) {
    if (i.isInstancedMesh) {
      ...
      u.needsUpdate = true, i.frustumCulled = false, s.instanced += f;
    } else {
      const f = i.geometry.clone().applyMatrix4(l), u = jn(f, t);
      f.dispose(), i.geometry = u, i.frustumCulled = true, s.wrapped++, s.tris += u.attributes.position.count / 3;
    }
...
function $n(e) {
  const t = e.attributes.position, o = { x: 0, z: 0, y: 0 }, n = new a.Vector3(), r = (c) => (n.set(t.getX(c), t.getY(c), t.getZ(c)), Xo(n, o), Wn()), s = [];
  for (let c = 0; c < t.count; c += 3) if (!(r(c) || r(c + 1) || r(c + 2))) for (let i = 0; i < 3; i++) s.push(t.getX(c + i), t.getY(c + i), t.getZ(c + i));
  e.setAttribute("position", new a.Float32BufferAttribute(s, 3));
}
function Yn(e) {
  const t = new a.Group();
  t.name = "planet";
  const o = new a.IcosahedronGeometry(Y, 30), n = o.attributes.position, r = new a.Vector3(), s = { x: 0, z: 0, y: 0 }, c = new a.Vector3();
  for (let l = 0; l < n.count; l++) {
    r.set(n.getX(l), n.getY(l), n.getZ(l)).normalize(), c.copy(r).multiplyScalar(Y).add(ot), Xo(c, s);
    const f = qn() + et() * 0.999 - On - 0.065;
    c.copy(r).multiplyScalar(Y + f).add(ot), n.setXYZ(l, c.x, c.y, c.z);
  }
  ...
  const i = new a.Mesh(o, b({ color: 12563607, bands: 4, tint: 8024982, flat: false }));
...
function wt(e, { thickness: t = 38e-4, color: o = g.ink, opacity: n = 1 } = {}) {
  if (!e || !e.geometry) return null;
  const r = new a.ShaderMaterial({ uniforms: { uThickness: { value: t }, uColor: { value: new a.Color(o) }, uOpacity: { value: n }, uResolution: { value: Zo.clone() } }, vertexShader: Xn, fragmentShader: Qn, side: a.BackSide, transparent: n < 1, depthWrite: true, fog: false });
  Jo.add(r);
  const s = Zn(e.geometry);
  let c;
  return e.isInstancedMesh ? (c = new a.InstancedMesh(s, r, e.count), c.instanceMatrix = e.instanceMatrix, c.count = e.count) : c = new a.Mesh(s, r), c.castShadow = false, c.receiveShadow = false, c.renderOrder = (e.renderOrder || 0) - 1, c.frustumCulled = e.frustumCulled, e.add(c), c;
}
const Jn = 1.62, ta = 0.34, ea = 0.38, Ot = { eye: 1.4, seatFwd: 0.46, nose: 0.92, noseR: 0.3, steer: 1.75 };
class oa {
  constructor(t, o, n, r = {}) {
    this.camera = t, this.dom = o, this.world = n, this.spawn = { pos: new a.Vector3(2.2, 0, -6.5), yaw: r.yaw ?? Math.PI + 0.14, pitch: r.pitch ?? -0.01 }, r.pos && this.spawn.pos.copy(r.pos), this.pos = this.spawn.pos.clone(), this.yaw = this.spawn.yaw, this.pitch = this.spawn.pitch, this.vel = new a.Vector3(), this.bob = 0, this.locked = false, this.keys = /* @__PURE__ */ new Set(), this.walkSpeed =
  ...
  _resolveAt(t, o, n, r) {
    for (const s of o) {
      if (s.top !== void 0 && s.top <= n + ea || s.bottom !== void 0 && s.bottom > n + 1.9) continue;
      const c = s.x0 - r, i = s.x1 + r, l = s.z0 - r, f = s.z1 + r;
  ...
  update(t) {
    const o = this.keys, n = this.ride !== null, r = o.has("ShiftLeft") || o.has("ShiftRight"), s = n ? this.rideSpeed : r ? this.runSpeed : this.walkSpeed;
    let c = 0, i = 0;
    ...
    this.yaw)), n ? this._wish.copy(this._forward).multiplyScalar(c > 0 ? s : c < 0 ? -1.7 : 0) : (this._wish.copy(this._forward).multiplyScalar(c).addScaledVector(this._right, i), this._wish.lengthSq() > 1e-6 && this._wish.normalize().multiplyScalar(s));
    ...
    this.roll += (S - this.roll) * (1 - Math.exp(-7 * t)), this.bob += t * A * (r ? 8.2 : 6.4), this.applyCamera(A);
  }
  applyCamera(t) {
    const o = this.ride !== null, n = o ? 0 : Math.min(t / this.walkSpeed, 1) * 0.014, r = this.pos.y + (o ? Ot.eye : Jn) + Math.sin(this.bob) * n, s = te(this.pos.x, this.pos.z, this._up, this._east, this._north);
    this._basis.makeBasis(this._east, this._up, this._north), this._surfaceQ.setFromRotationMatrix(this._basis), this._localE.set(this.pitch, this.yaw, this.roll + Math.sin(this.bob * 0.5) * n * 0.35, "YXZ"), this._localQ.setFromEuler(this._localE), Yt(this.pos.x, r, this.pos.z, this.camera.position), this.camera.quaternion.copy(this._surfaceQ).multiply(this._localQ), this.camera.up.copy(s.up);
  }
  pick(t) {
...
function na({ volume: e = 0.34 } = {}) {
  ...
  }, o = t("div", "hud"), n = t("div", "crosshair", o), r = t("div", "prompt", o, ""), s = t("div", "toast", o, ""), c = t("div", "memory-card", o, "");
  ...
  const v = { root: o, overlay: h, onStart: null, onVolumeChange: null, flash(z, R = 1400) {
    s.textContent = z, s.classList.add("on"), clearTimeout(S), S = setTimeout(() => s.classList.remove("on"), R);
  }, setPrompt(z) {
    z ? (r.textContent = z, r.classList.add("on")) : r.classList.remove("on");
  }, setPlanetView(z) {
  ...
  return w.addEventListener("input", () => {
    ...
    L.addEventListener("touchstart", (B) => {
      B.touches.length === 1 && (V = { x: B.touches[0].clientX, y: B.touches[0].clientY }, H?.());
    }, { passive: true }), L.addEventListener("touchmove", (B) => {
      if (!V || B.touches.length !== 1) return;
      const E = B.touches[0];
      z.touchLook(E.clientX - V.x, E.clientY - V.y), V = { x: E.clientX, y: E.clientY }, B.preventDefault();
...
function aa({ volume: e = 0.34, fadeIn: t = 2.6 } = {}) {
  ...
  function M() {
    ne.length && (c = s(), i.src = ne[c], i.load());
  }
  function G(x, S) {
...
dm: 2.4, cr: 6.4, fa: 0.44 }, { x: 199, z: -39.4, bank: 0.28, dr: 20, dm: 2.5, cr: 6.8, fa: 0.46 }, { x: 186, z: -39.6, bank: 0.28, dr: 20, dm: 2.5, cr: 7.2, fa: 0.46 }, { x: 174, z: -40.6, bank: 0.3, dr: 18, dm: 2.5, cr: 7.2, fa: 0.46 }, { x: 166, z: -42.4, bank: 0.36, dr: 16, dm: 2.5, cr: 6, fa: 0.46 }], Qe = ze.map((e, t) => {
  const o = ze[(t + 1) % ze.length], n = o.x - e.x, r = o.z - e.z;
...
const Ft = (() => {
  ...
  for (const r of ze) e = Math.min(e, r.x), t = Math.max(t, r.x), o = Math.min(o, r.z), n = Math.max(n, r.z);
...
function on(e, t) {
  ...
  const s = Math.sqrt(o), c = en(e, t);
  if (!c && s > ae) return null;
  const { a: i, b: l } = n, f = (u, d) => u + (d - u) * r;
  return { d: c ? s : -s, bank: f(i.bank, l.bank), dr: f(i.dr, l.dr), dm: f(i.dm, l.dm), cr: f(i.cr, l.cr), fa: f(i.fa, l.fa), hi: f(i.hi ?? co, l.hi ?? co), ho: f(i.ho ?? lo, l.ho ?? lo), arc: n.s0 + n.len * r };
}
function ca(e, t) {
...
function la(e, t, o) {
  ...
  const n = -o.d, r = Math.sin(o.arc * 0.062) * 4 + Math.sin(o.arc * 0.148 + 1.7) * 1.8, s = Math.max(2, ia + r), c = o.cr * (1 + 0.09 * Math.sin(o.arc * 0.091 + 0.4)), i = c / Math.max(0.08, o.bank);
  return n <= i ? rt + o.bank * n : n <= i + s ? rt + c : rt + c - (n - i - s) * o.fa;
}
...
function ua(e, t) {
  ...
  for (const n of fa) {
    ...
    r = r < 0 ? 0 : r > 1 ? 1 : r;
    const s = n.d.a[0] + n.dx * r, c = n.d.a[1] + n.dz * r, i = Math.hypot(e - s, t - c), l = i <= n.d.half ? n.d.crest : n.d.crest - (i - n.d.half) * n.d.face;
    l > o && (o = l);
  }
  return o;
}
...
for (const e of ha) for (let t = 0; t < e.pts.length - 1; t++) {
  const o = e.pts[t], n = e.pts[t + 1], r = n[0] - o[0], s = n[1] - o[1];
  nn.push({ c: e, a: o, b: n, dx: r, dz: s, l2: r * r + s * s || 1e-6 });
}
function pa(e, t) {
  let o = 1 / 0;
  for (const n of nn) {
    ...
    r = r < 0 ? 0 : r > 1 ? 1 : r;
    const s = n.a[0] + n.dx * r, c = n.a[1] + n.dz * r, i = Math.hypot(e - s, t - c);
...
function ma(e, t, o, n) {
  ...
  const f = i(la(t, o, r));
  if (f > s && (s = f), r.d >= 0) {
    const d = Dt(r.d / r.dr, 0, 1);
    s = rt - r.dm * d * d * (3 - 2 * d);
  } else {
    const d = -r.d, h = rt + r.bank * d, p = h < s ? h : s, m = yt(r.hi, r.ho, d);
    s = p + (s - p) * m;
  }
  const u = pa(t, o);
  return u < s ? u : s;
...
function xa(e, t, o) {
  ...
  if (!n) return 1;
  if (n.d > -2) return 0;
  const r = Dt((-n.d - 2) / 10, 0, 1), s = Dt((o - rt - 0.9) / 2.4, 0, 1);
  return r < s ? r : s;
}
...
function ga(e, t) {
  let o = 1;
  for (const n of ya) {
    const r = (e - n.x) / n.rx, s = (t - n.z) / n.rz, c = r * r + s * s;
    if (c >= 1) continue;
    const i = n.h * (1 - c) * (1 - c);
...
function va(e, t) {
  ...
  if (r <= 0) return 0;
  for (const s of an) {
    const c = Math.max(s.x0 - e, e - s.x1, 0), i = Math.max(s.z0 - t, t - s.z1, 0);
    if (r *= yt(0, s.r, Math.hypot(c, i)), r <= 0) return 0;
  }
  return r;
}
function ka(e, t) {
...
const Ga = (() => {
  const e = nt(778213), t = [];
  for (let n = 0; n < 170; n++) {
    const r = e.range(-166, 166), s = e.range(-190, 104), c = e.range(7, 21);
    t.push({ x: r, z: s, rx: c * e.range(0.7, 1.4), rz: c * e.range(0.7, 1.4), h: e.range(0.55, 2.05) * (e.chance(0.34) ? -1 : 1) });
  }
  const o = nt(551907);
  for (let n = 0; n < 76; n++) {
    const r = o.range(158, 302), s = o.range(-190, 104), c = o.range(7, 21);
    t.push({ x: r, z: s, rx: c * o.range(0.7, 1.4), rz: c * o.range(0.7, 1.4), h: o.range(0.55, 2.05) * (o.chance(0.34) ? -1 : 1) });
  }
  return t;
})();
function Ta(e, t) {
...
for (const e of Aa) {
  const t = Math.floor((e.x - e.rx) / Qt), o = Math.floor((e.x + e.rx) / Qt), n = Math.floor((e.z - e.rz) / Qt), r = Math.floor((e.z + e.rz) / Qt);
  for (let s = t; s <= o; s++) for (let c = n; c <= r; c++) {
    const i = ue(s, c);
    let l = Ue.get(i);
...
function Ca(e, t) {
  ...
  let n = 0;
  for (const r of o) {
    const s = (e - r.x) / r.rx, c = (t - r.z) / r.rz, i = s * s + c * c;
    i >= 1 || (n += r.h * (1 - i) * (1 - i));
  }
  return n;
}
...
for (const e of Ra) {
  const t = Math.floor((e.x - e.rx) / Zt), o = Math.floor((e.x + e.rx) / Zt), n = Math.floor((e.z - e.rz) / Zt), r = Math.floor((e.z + e.rz) / Zt);
  for (let s = t; s <= o; s++) for (let c = n; c <= r; c++) {
    const i = ue(s, c);
    let l = je.get(i);
...
for (const e of Ba) {
  const t = Math.floor((e.x - e.rx) / xe), o = Math.floor((e.x + e.rx) / xe), n = Math.floor((e.z - e.rz) / xe), r = Math.floor((e.z + e.rz) / xe);
  for (let s = t; s <= o; s++) for (let c = n; c <= r; c++) {
    const i = ue(s, c);
    let l = ho.get(i);
...
252.6, -89], [252.8, -91.4]] } }, rn = (() => {
  const e = [], t = (o, n, r, s, c) => {
    ...
    e.push({ a: o, b: n, dx: i, dz: l, l2: i * i + l * l || 1e-6, flat: r, fade: s, given: c });
  };
  for (const o of Object.keys(po)) {
    const n = po[o];
    for (let r = 0; r < n.length - 1; r++) t(n[r], n[r + 1], Ea, Ia, false);
  }
  for (const o of Object.keys(mo)) {
    const n = mo[o], r = n.pts[0].length > 2;
    for (let s = 0; s < n.pts.length - 1; s++) t(n.pts[s], n.pts[s + 1], n.flat, n.fade, r);
  }
  return e;
})();
function cn(e, t) {
  ...
  for (const s of rn) {
    let c = ((e - s.a[0]) * s.dx + (t - s.a[1]) * s.dz) / s.l2;
    c = c < 0 ? 0 : c > 1 ? 1 : c;
    const i = Math.hypot(e - (s.a[0] + s.dx * c), t - (s.a[1] + s.dz * c));
    i < o && (o = i, n = s, r = c);
  }
  return { d: o, seg: n, t: r };
}
function Da(e, t) {
  const o = cn(e, t), n = o.seg.flat + 0.1, r = n + 2;
...
const Ne = -1.3 + 5e-3;
{
  ...
  for (let o = ft; o <= Ut; o++) for (let n = ut; n <= jt; n++) {
    const r = o * dt, s = n * dt, c = va(r, s);
    kt[(o - ft) * xt + (n - ut)] = e(r, s) ? -1.3 : Math.max(-1.3, ma(ga(r, s) * c * ba(s) - wa, r, s, c));
  }
  const t = new Float32Array(ve * xt);
  for (let o = ft; o <= Ut; o++) for (let n = ut; n <= jt; n++) t[(o - ft) * xt + (n - ut)] = _a(o * dt, n * dt) * dt;
  for (let o = 0; o < 140; o++) {
    let n = false;
    for (let r = 0; r < ve; r++) for (let s = 0; s < xt; s++) {
      const c = r * xt + s, i = kt[c];
      if (i <= Ne) continue;
      let l = 1 / 0;
      for (let f = 0; f < 4; f++) {
        const u = r + (f === 0 ? 1 : f === 1 ? -1 : 0), d = s + (f === 2 ? 1 : f === 3 ? -1 : 0);
  ...
  for (let o = ft; o <= Ut; o++) for (let n = ut; n <= jt; n++) {
    const r = (o - ft) * xt + (n - ut), s = kt[r];
    if (s <= Ne) continue;
    const c = o * dt, i = n * dt;
    if (e(c, i)) continue;
    const l = Math.max(0, Math.min(1, s / 2.4)), f = Math.max(0, Math.min(1, s / 1.6)), u = Math.max(0, Math.min(1, s / 0.7)), d = xa(c, i, s), h = Da(c, i) * d;
    let p = Ta(c, i) * l * (0.2 + 0.8 * d) + (Sa(o, n) * 0.21 * l + Ca(c, i) * f + Pa(c, i) * u) * h;
    s > 0 && p < -0.75 * s && (p = -0.75 * s), kt[r] = Math.max(-1.3, s + p);
  }
  {
    const o = /* @__PURE__ */ new Map(), n = (r) => r[0] + "," + r[1];
    for (const r of rn) for (const s of [r.a, r.b]) o.has(n(s)) || o.set(n(s), r.given ? s[2] : Na(s[0], s[1]));
    for (let r = ft; r <= Ut; r++) for (let s = ut; s <= jt; s++) {
      const c = (r - ft) * xt + (s - ut), i = kt[c];
      if (i <= Ne) continue;
      const l = r * dt, f = s * dt;
      if (e(l, f)) continue;
      const u = cn(l, f);
...
function Na(e, t) {
  ...
  const r = e / dt - o, s = t / dt - n, c = we(o, n), i = we(o + 1, n), l = we(o, n + 1), f = we(o + 1, n + 1);
  return La(o, n) ? r + s <= 1 ? c + (i - c) * r + (l - c) * s : f + (f - l) * (r - 1) + (f - i) * (s - 1) : r >= s ? c + (i - c) * r + (f - i) * s : c + (f - l) * r + (l - c) * s;
}
function Oa(e, t) {
...
const se = 1.44, Fa = 0.3, Mt = -ht / 2, Ct = ht / 2, xo = -150, wo = 150, yo = () => b({ color: g.railMetal, bands: 3, tint: 6248568, flat: false }), bo = () => b({ color: g.railHead, bands: 2, tint: 7301264 }), Va = () => b({ color: g.sleeper, bands: 3, tint: 6117496 }), Wa = () => b({ color: g.ballast, bands: 3, tint: 6643076 }), Ha = () => b({ color: g.gateYellow, bands: 3, tint: 9400400 }), Ua = () => b(
{ color: g.gateBlack, bands: 2, tint: 4932960 }), ln = () => b({ color: g.metal, bands: 3, tint: 6709392 }), dn = () => b({ color: g.metalDark, bands: 3, tint: 6051456 }), go = () => b({ color: g.cabinet, bands: 3, tint: 7301264 }), ja = () => b({ color: g.concrete, bands: 3, tint: 7301008 });
function Ka(e) {
  const t = new a.Group();
  t.name = "railway", e.add(t);
  {
    const s = new a.Shape();
    s.moveTo(-bt - 0.9, 0), s.lineTo(-bt, 0.26), s.lineTo(bt, 0.26), s.lineTo(bt + 0.9, 0), s.closePath();
    const c = new a.ExtrudeGeometry(s, { depth: Ct - Mt, bevelEnabled: false });
    c.rotateY(Math.PI / 2), c.translate(Mt, 0, 0);
    const i = new a.Mesh(c, Wa());
    i.receiveShadow = true, i.name = "ballast", t.add(i);
  }
  {
    const s = new a.BoxGeometry(0.24, 0.16, 2.5), c = Math.floor((Ct - Mt) / 0.62), i = new a.InstancedMesh(s, Va(), c), l = new a.Object3D();
    let f = 0;
    for (let u = Mt; u < Ct; u += 0.62) Math.abs(u) < F + K + 0.6 || (l.position.set(u, 0.32, 0), l.rotation.set(0, 0, 0), l.updateMatrix(), i.setMatrixAt(f++, l.matrix));
    i.count = f, i.receiveShadow = true, i.castShadow = true, t.add(i);
  }
  {
    const s = Ct - Mt, c = pt([{ geometry: new a.BoxGeometry(s, 0.055, 0.115), matrix: T(0, 0.392, 0) }, { geometry: new a.BoxGeometry(s, 0.1, 0.05), matrix: T(0, 0.325, 0) }, { geometry: new a.BoxGeometry(s, 0.03, 0.17), matrix: T(0, 0.27, 0) }]);
    for (const i of [-1, 1]) {
      const l = new a.Mesh(c.clone(), yo());
      l.position.set((Mt + Ct) / 2, 0, i * se / 2), l.castShadow = true, l.receiveShadow = true, l.name = "rail", t.add(l);
      const f = D(s, 0.016, 0.09, bo(), (Mt + Ct) / 2, 0.424, i * se / 2);
      f.userData.noOutline = true, t.add(f);
    }
    c.dispose();
  }
  {
    const s = F * 2 + K * 2 + 1, c = U(0), i = b({ color: 14341599, bands: 3, tint: 7301008 }), l = D(s, 0.28, se - 0.26, i, c, 0.18, 0);
    l.receiveShadow = true, t.add(l);
    for (const u of [-1, 1]) {
      const d = D(s, 0.28, 1.55, i, c, 0.18, u * (se / 2 + 0.12 + 0.78));
      d.receiveShadow = true, t.add(d);
    }
    for (const u of [-1, 1]) {
      const d = u * se / 2, h = D(s, 0.12, 0.115, yo(), c, 0.36, d);
      h.receiveShadow = true, t.add(h);
      const p = D(s, 0.016, 0.09, bo(), c, 0.424, d);
      p.userData.noOutline = true, t.add(p);
      for (const m of [-1, 1]) t.add(D(s, 0.06, 0.055, b({ color: 5064535, bands: 2, tint: 4275288 }), c, 0.33, d + m * 0.086));
    }
    const f = b({ color: g.lineYellow, bands: 2, tint: 9400400 });
    for (const u of [-1, 1]) {
      for (const [d, h] of [[0.42, 0.3], [0.92, 0.16]]) {
        const p = new a.PlaneGeometry(F * 2 - 0.15, h);
        p.rotateX(-Math.PI / 2);
  ...
  const o = bt + 1.05, n = F + K + 0.5;
  {
    const s = [], c = new a.BoxGeometry(1, 0.06, 0.06), i = new a.BoxGeometry(0.07, 1.12, 0.07), l = new a.BoxGeometry(0.035, 0.5, 0.035), f = (h, p, m) => {
      let w = [[p, m]];
      const M = [];
    ...
    for (const [h, p, m] of u) {
      const w = m - p, M = (p + m) / 2;
      for (const G of [0.55, 1.02]) s.push({ geometry: c, matrix: T(M, G, h, 0, 0, 0, w, 1, 1) });
      for (let G = p; G <= m; G += 2.4) s.push({ geometry: i, matrix: T(G, 0.56, h) });
      for (let G = p + 0.3; G <= m; G += 0.32) s.push({ geometry: l, matrix: T(G, 0.78, h) });
      e.collide(p, h - 0.12, m, h + 0.12, 1.2);
    }
    const d = new a.Mesh(pt(s), ln());
    d.castShadow = true, d.name = "linesideFence", t.add(d), [c, i, l].forEach((h) => h.dispose());
  }
  {
    const s = dn(), c = [], i = ht / Math.round(ht / 19), l = 0.02;
    ...
    const f = new a.Mesh(pt(c), s);
    f.castShadow = true, t.add(f);
    const u = b({ color: g.metalDark, bands: 2, tint: 4867176 });
  ...
  $a(e, t);
  {
    const s = b({ color: g.concreteMid, bands: 3, tint: 6972040 }), c = [[bt + 2.6, -80, -30], [bt + 2.6, 46, 80], [-4.800000000000001, -80, -30], [-4.800000000000001, 44, 80]], i = b({ color: g.concrete, bands: 3, tint: 7301008 });
    for (const [l, f, u] of c) {
      const d = D(u - f, 2.2, 0.35, s, (f + u) / 2, 1.1, l);
      d.castShadow = true, d.receiveShadow = true, t.add(d), e.collide(f, l - 0.2, u, l + 0.2, 2.2);
      for (const h of [f, u]) {
        const m = h - (h === f ? 1 : -1) * 0.22, w = D(0.52, 2.52, 0.62, s, m, 1.26, l);
...
function qa(e, t) {
  ...
  const n = U(0), r = Ha(), s = Ua(), c = ln(), i = dn(), l = go(), f = [], u = [];
  function d(x) {
    ...
    const R = new a.Mesh(pt(v), r), O = new a.Mesh(pt(z), s);
    R.castShadow = O.castShadow = true, S.add(R, O);
    for (let H = 1; H < k - 1; H += 3) {
      const C = D(0.11, 0.11, 0.07, I({ color: g.signalOff }), y * (H + 0.5), -0.14, 0.06);
  ...
  function h(x, S, y) {
    const k = new a.Group(), P = n + x * (F + 0.42), v = S * ke;
    ...
    if (z.receiveShadow = z.castShadow = true, k.add(z), y === "arm") {
      ...
      B.castShadow = B.receiveShadow = true, k.add(B), wt(B, { thickness: 34e-4 }), k.add(D(0.54, 0.07, 0.46, b({ color: g.cabinetTop, bands: 3 }), 0, 0.2 + 0.92 + 0.03, 0));
      for (let j = 0; j < 3; j++) k.add(D(0.48, 0.11, 0.4, j % 2 ? s : r, 0, 0.34 + j * 0.24, 0));
      const E = new a.Group();
      E.position.set(0, 0.2 + 0.92 + 0.12, S * 0.2), E.add(d(F * 2 + 0.5)), E.rotation.y = x > 0 ? Math.PI : 0, E.rotation.z = Math.PI / 2, k.add(E), u.push({ pivot: E }), k.add(D(0.2, 0.2, 0.14, i, 0, 0.2 + 0.92 + 0.12, S * 0.2));
    }
    const R = y === "arm" ? x * 0.44 : 0, O = 2.45, H = Jt(0.075, 0.09, O, 8, r, R, 0.2 + O / 2, 0);
    H.castShadow = true, k.add(H);
    for (let V = 0; V < 4; V++) k.add(Jt(0.082, 0.082, 0.22, 8, s, R, 0.42 + V * 0.56, 0));
    wt(H, { thickness: 32e-4 }), y === "arm" && k.add(D(0.5, 0.09, 0.09, i, R / 2, 0.2 + 0.5, 0));
    const C = new a.Group();
    C.position.set(R, 0.2 + O + 0.02, S * 0.02), C.rotation.y = S > 0 ? 0 : Math.PI, C.add(D(0.86, 0.13, 0.1, s, 0, 0.06, 0));
    for (const V of [-0.28, 0.28]) {
      const B = new a.Mesh(new a.CylinderGeometry(0.145, 0.16, 0.13, 12, 1, true), s);
      B.rotation.x = Math.PI / 2, B.position.set(V, -0.12, 0.07), C.add(B);
      const E = new a.Mesh(new a.CircleGeometry(0.16, 14), s);
      E.position.set(V, -0.12, 0), C.add(E);
      const j = new a.Mesh(new a.CircleGeometry(0.13, 14), I({ color: g.signalOff, cache: false }));
      j.position.set(V, -0.12, 0.135), j.userData.lamp = V < 0 ? "a" : "b", f.push(j), C.add(j);
    }
    ...
    if (L.rotation.x = Math.PI, L.position.set(0, 0.3, 0), C.add(L), k.add(C), y === "sign") {
      const V = [I({ color: g.wallWhite }), I({ color: g.wallWhite }), I({ color: g.wallWhite }), I({ color: g.wallWhite }), I({ color: 16777215, map: Cn(), cache: false }), I({ color: g.wallGray })], B = new a.Mesh(new a.BoxGeometry(1.15, 0.58, 0.05), V);
      ...
      for (const E of [0.72, -0.72]) {
        const j = D(1.2, 0.15, 0.045, r, 0, 0.2 + O + 1.28, S * 0.05);
        j.rotation.z = E, j.castShadow = true, k.add(j);
      }
      k.add(Jt(0.06, 0.07, 1.3, 8, r, 0, 0.2 + O + 0.65, 0));
    }
    return o.add(k), e.collide(P - 0.4, v - 0.35, P + 0.4, v + 0.35, 2.4), k;
  }
  h(-1, 1, "arm"), h(1, -1, "arm"), h(1, 1, "sign"), h(-1, -1, "sign");
  {
    const x = -ke - 1.15, S = n - (F + 1.5), y = D(0.78, 1.32, 0.5, l, S, et() + 0.66, x);
    y.castShadow = y.receiveShadow = true, o.add(y), wt(y, { thickness: 3e-3 });
    const k = D(0.86, 0.08, 0.58, b({ color: g.cabinetTop, bands: 3 }), S, et() + 1.36, x);
    o.add(k);
    const P = D(0.52, 0.9, 0.4, l, S - 0.9, et() + 0.45, x + 0.1);
    P.castShadow = P.receiveShadow = true, o.add(P), o.add(D(0.6, 0.06, 0.46, b({ color: g.cabinetTop, bands: 3 }), S - 0.9, et() + 0.93, x + 0.1)), o.add(D(0.02, 1, 0.02, b({ color: g.metalDark, bands: 2 }), S, et() + 0.66, x - 0.26));
    const v = D(0.07, 0.07, 0.03, I({ color: g.signalRed }), S + 0.24, et() + 1.16, x - 0.26);
    o.add(v), e.collide(S - 1.3, x - 0.4, S + 0.5, x + 0.4, et() + 1.4);
    const z = D(0.3, 0.4, 0.12, b({ color: g.yellow, bands: 3 }), S, et() + 1, x - 0.3);
  ...
  {
    const x = b({ color: g.concrete, bands: 3, tint: 7301008 });
    for (const S of [-1, 1]) {
      const y = n + S * (F + 0.15), k = D(0.34, 0.16, bt * 2 + 1.6, x, y, et() + 0.08, 0);
      k.receiveShadow = true, k.castShadow = true, o.add(k);
      for (const P of [-1, 1]) {
        const v = new a.Mesh(new a.PlaneGeometry(1.35, 0.6), b({ color: 16777215, bands: 2, map: qo(true), tint: 9400400, cache: false }));
        v.rotation.x = -Math.PI / 2, v.position.set(n + S * (F + 0.82), et() + Lt + 0.016, P * (It + 0.32)), o.add(v);
      }
    }
  }
  const p = new a.Color(g.signalRed), m = new a.Color(g.signalOff), w = f.filter((x) => x.userData.lamp === "arm"), M = f.filter((x) => x.userData.lamp === "a"), G = f.filter((x) => x.userData.lamp === "b");
...
function $a(e, t, o) {
  ...
  const r = 15.5, s = 38, c = -1.92, i = c - 3.7, l = 0.98, f = b({ color: g.concrete, bands: 3, tint: 7301008 }), u = b({ color: g.concreteMid, bands: 3, tint: 6972040 }), d = D(s - r, l, c - i, f, (r + s) / 2, l / 2, (c + i) / 2);
  d.castShadow = d.receiveShadow = true, n.add(d), e.platform({ x0: r, x1: s, z0: i, z1: c, top: l }), e.collide(r - 0.1, i - 0.1, s + 0.1, c + 0.1, l), n.add(D(s - r, 0.06, 0.34, u, (r + s) / 2, l - 0.02, c - 0.17));
  const h = new a.Mesh(new a.PlaneGeometry(s - r - 1, 0.46), b({ color: 16777215, bands: 2, map: qo(), tint: 9400400, cache: false }));
  h.material.map.repeat.set((s - r - 1) / 0.46, 1), h.material.map.wrapS = a.RepeatWrapping, h.rotation.x = -Math.PI / 2, h.position.set((r + s) / 2, l + 0.012, c - 0.62), n.add(h);
  {
    const m = (r + s) / 2 + 1.5, w = 9.5, M = 3.2, G = [];
    ...
    const A = new a.Mesh(pt(G), b({ color: g.metalDark, bands: 3 }));
    A.castShadow = true, n.add(A);
    const x = D(w, 0.16, M, b({ color: g.roofTeal, bands: 3, tint: 4867176 }), m, l + 2.66, (c + i) / 2 - 0.1);
    x.castShadow = x.receiveShadow = true, n.add(x), n.add(D(w + 0.3, 0.1, 0.14, b({ color: g.metal, bands: 3 }), m, l + 2.56, (c + i) / 2 - 1.75)), wt(x, { thickness: 3e-3 });
    for (const S of [m - 2.2, m + 1.4]) {
      const y = new a.Group();
      y.add(D(1.7, 0.08, 0.42, b({ color: g.wallCream, bands: 3 }), 0, 0.42, 0)), y.add(D(1.7, 0.5, 0.07, b({ color: g.wallCream, bands: 3 }), 0, 0.66, -0.2));
      for (const k of [-0.7, 0.7]) y.add(D(0.1, 0.42, 0.36, b({ color: g.metalDark, bands: 3 }), k, 0.21, 0));
      y.position.set(S, l, i + 0.95), y.traverse((k) => {
        k.isMesh && (k.castShadow = true);
      }), n.add(y);
    }
  }
  for (const m of [r + 4, s - 4.5]) {
    const w = Jt(0.06, 0.06, 2.2, 8, b({ color: g.metalDark, bands: 3 }), m, l + 1.1, c - 0.55);
    w.castShadow = true, n.add(w);
    const M = new a.Mesh(new a.BoxGeometry(1.9, 0.5, 0.06), [I({ color: g.wallWhite }), I({ color: g.wallWhite }), I({ color: g.wallWhite }), I({ color: g.wallWhite }), I({ color: 16777215, map: Rn(), cache: false }), I({ color: g.wallGray })]);
    M.position.set(m, l + 2.25, c - 0.55), M.castShadow = true, n.add(M), wt(M, { thickness: 3e-3 });
  }
  {
    const m = [];
    for (let M = r; M <= s; M += 2.2) m.push({ geometry: new a.BoxGeometry(0.08, 1.2, 0.08), matrix: T(M, l + 0.6, i + 0.06) });
    for (const M of [l + 0.5, l + 1.1]) m.push({ geometry: new a.BoxGeometry(s - r, 0.06, 0.06), matrix: T((r + s) / 2, M, i + 0.06) });
    const w = new a.Mesh(pt(m), b({ color: g.metal, bands: 3 }));
    w.castShadow = true, n.add(w);
  }
  {
    for (let w = 0; w < 6; w++) {
      const M = l * ((6 - w) / 6), G = r - 0.18 - w * 0.36, A = D(0.36, M, 2.4, f, G, M / 2, i + 1.3);
    ...
    for (const w of [i + 0.1, i + 2.5]) {
      const M = D(2.4, 0.07, 0.07, b({ color: g.metal, bands: 3 }), r - 1.2, 0.95, w);
      M.rotation.z = 0.36, n.add(M);
    }
  }
  for (const m of [r + 2.5, (r + s) / 2 - 3.5, s - 2]) {
    const w = Jt(0.055, 0.055, 3.1, 8, b({ color: g.metalDark, bands: 3 }), m, l + 1.55, i + 0.5);
    w.castShadow = true, n.add(w);
    const M = new a.Mesh(new a.ConeGeometry(0.26, 0.22, 10, 1, true), b({ color: g.wallGray, bands: 3 }));
    M.position.set(m, l + 3.05, i + 0.5), n.add(M), n.add(D(0.18, 0.05, 0.18, I({ color: 16774100 }), m, l + 2.92, i + 0.5));
  }
  const p = new a.Mesh(new a.BoxGeometry(0.36, 0.72, 0.04), [I({ color: g.wallGray }), I({ color: g.wallGray }), I({ color: g.wallGray }), I({ color: g.wallGray }), I({ color: 16777215, map: Ko(1), cache: false }), I({ color: g.wallGray })]);
...
function Xa() {
  W.body || (W.body = b({ color: g.trainBody, bands: 3, tint: 7301014 }), W.bodyShade = b({ color: g.trainBodyShade, bands: 3, tint: 7301014 }), W.stripe = b({ color: g.trainStripe, bands: 3, tint: 4868754 }), W.stripe2 = b({ color: g.trainStripe2, bands: 3, tint: 4151946 }), W.roof = b({ color: g.trainRoof, bands: 3, tint: 6314367 }), W.skirt = b({ color: g.trainSkirt, bands: 3, tint: 5985408 }),
  W.window = I({ color: g.trainWindow }), W.windowLit = I({ color: g.trainWindowLit }), W.door = b({ color: g.trainDoor, bands: 3, tint: 7301014 }), W.dark = b({ color: g.black, bands: 2, tint: 4932960 }), W.metal = b({ color: g.metalDark, bands: 3, tint: 6051456 }), W.wheel = b({ color: 4867410, bands: 2, tint: 4932960 }), W.headlight = I({ color: 16774874 }), W.tail = I({ color: 16734794 }));
}
...
function ts({ cab: e = false, tail: t = false, rng: o }) {
  const n = new a.Group(), r = { body: [], stripe: [], roof: [], skirt: [], door: [], dark: [], metal: [] }, s = zt - Pt;
  r.body.push({ geometry: new a.BoxGeometry(Rt, s, st), matrix: T(0, (Pt + zt) / 2, 0) }), r.roof.push({ geometry: new a.BoxGeometry(Rt - 0.1, re - zt, st - 0.24), matrix: T(0, (zt + re) / 2, 0) });
  for (const d of [-1, 1]) r.roof.push({ geometry: new a.BoxGeometry(Rt - 0.05, 0.07, 0.1), matrix: T(0, zt + 0.02, d * (st / 2 - 0.06)) });
  const c = 1.92, i = 0.34;
  r.stripe.push({ geometry: new a.BoxGeometry(Rt + 0.02, i, st + 0.03), matrix: T(0, c, 0) }), r.stripe.push({ geometry: new a.BoxGeometry(Rt + 0.02, 0.07, st + 0.04), matrix: T(0, c - i / 2 - 0.055, 0) }), r.skirt.push({ geometry: new a.BoxGeometry(Rt - 0.3, 0.5, st - 0.34), matrix: T(0, Pt - 0.25, 0) }), r.skirt.push({ geometry: new a.BoxGeometry(Rt - 1.6, 0.28, st - 0.8), matrix: T(0, Pt - 0.52,
  0) });
  for (const d of [1, -1]) {
    for (const h of Qa) r.door.push({ geometry: new a.BoxGeometry(Za, zt - Pt - 0.12, 0.05), matrix: T(h, (Pt + zt) / 2 - 0.02, d * (st / 2 + 0.012)) }), r.dark.push({ geometry: new a.BoxGeometry(0.05, zt - Pt - 0.12, 0.06), matrix: T(h, (Pt + zt) / 2 - 0.02, d * (st / 2 + 0.02)) }), zo(n, h, 0.94, d, o);
    for (const [h, p] of Ja) r.metal.push({ geometry: new a.BoxGeometry(p + 0.12, 1.14, 0.035), matrix: T(h, 2.66, d * (st / 2 + 8e-3)) }), zo(n, h, p, d, o);
  }
  for (const d of [-6.3, 6.3]) r.metal.push({ geometry: new a.BoxGeometry(2.9, 0.42, st - 0.9), matrix: T(d, 0.78, 0) }), r.dark.push({ geometry: new a.BoxGeometry(3.3, 0.2, 0.28), matrix: T(d, 0.62, 0) });
  for (const d of [-5.6, -1.4, 3.2, 7.4]) r.roof.push({ geometry: new a.BoxGeometry(2.1, 0.3, 1.5), matrix: T(d, re + 0.13, d % 2 === 0 ? 0.18 : -0.18) });
  for (const d of [-8.2, 0.6, 8.6]) r.metal.push({ geometry: new a.BoxGeometry(0.7, 0.16, 0.7), matrix: T(d, re + 0.07, -0.7) });
  if (e || t) {
    const d = e ? 1 : -1, h = d * (Rt / 2);
    r.dark.push({ geometry: new a.BoxGeometry(0.1, 1.34, st - 0.22), matrix: T(h + d * 0.03, 2.86, 0) });
    ...
    r.body.push({ geometry: new a.BoxGeometry(0.12, 1.4, 0.16), matrix: T(h + d * 0.05, 2.86, 0) });
    const p = new a.Mesh(new a.PlaneGeometry(1.5, 0.38), I({ color: 16777215, map: Pn(), cache: false }));
    p.position.set(h + d * 0.09, 3.52, 0), p.rotation.y = d > 0 ? Math.PI / 2 : -Math.PI / 2, p.userData.noOutline = true, n.add(p), r.dark.push({ geometry: new a.BoxGeometry(0.08, 0.5, 1.66), matrix: T(h + d * 0.04, 3.52, 0) });
    for (const w of [-1.06, 1.06]) {
      r.dark.push({ geometry: new a.BoxGeometry(0.14, 0.42, 0.5), matrix: T(h + d * 0.05, 1.55, w) });
    ...
    r.skirt.push({ geometry: new a.BoxGeometry(0.34, 0.78, st - 0.5), matrix: T(h + d * 0.1, 0.82, 0) }), r.dark.push({ geometry: new a.BoxGeometry(0.5, 0.22, 0.34), matrix: T(h + d * 0.25, 0.62, 0) });
...
function es(e) {
  ...
  for (let l = 0; l < 3; l++) {
    ...
    f.position.x = (l - 1) * Mo, o.add(f), r.push(f), n.push(...u);
  }
  const s = new a.Matrix4().makeTranslation(ot.x, ot.y, ot.z), c = new a.Matrix4().makeTranslation(-ot.x, -ot.y, -ot.z);
  return { group: o, cars: r, wheels: n, length: Mo * 3, dir: 1, x: 0, speed: 23.5, gust: 0, get offset() {
    return Qo(this.x, 0);
  }, planetize() {
    o.visible = true, o.matrixAutoUpdate = false, this.update(0);
  }, update(l) {
    this.x = gt(this.x + this.dir * this.speed * l), o.matrix.makeRotationZ(-this.x / Y).premultiply(s).multiply(c), o.matrixWorldNeedsUpdate = true;
...
function os(e) {
  const t = nt(8123), o = In(), n = new a.PlaneGeometry(0.185, 0.135), r = [{ color: g.petal, n: Math.round(ie * 0.55) }, { color: g.blossomLight, n: Math.round(ie * 0.28) }, { color: g.petalDeep, n: ie - Math.round(ie * 0.55) - Math.round(ie * 0.28) }], s = [], c = [];
  for (const p of r) {
    ...
    w.instanceMatrix.setUsage(a.DynamicDrawUsage), w.frustumCulled = false, w.renderOrder = 4, w.userData.noOutline = true, e.add(w), s.push(w);
  ...
  function h(p, m, w) {
    ...
    for (const A of s) A.instanceMatrix.needsUpdate = true;
  }
  for (let p = 0; p < 40; p++) h(0.1, 0, 1);
  return ns(e, o), { update: h, meshes: s };
}
function ns(e, t) {
  const o = nt(4471), n = new a.PlaneGeometry(0.17, 0.125);
  n.rotateX(-Math.PI / 2);
  const r = [g.petal, g.blossomLight, g.petalDeep], s = [[], [], []], c = new a.Object3D(), i = (l, f, u) => {
    c.position.set(l, u + 0.019, f), c.rotation.set(0, o.range(0, 6.28), 0);
    const d = o.range(0.8, 1.25);
    c.scale.set(d, 1, d), c.updateMatrix(), s[o.int(0, 2)].push(c.matrix.clone());
  };
  for (let l = 0; l < 620; l++) {
  ...
  s.forEach((l, f) => {
    if (!l.length) return;
    const u = new a.InstancedMesh(n, I({ color: r[f], map: t, transparent: true, opacity: 0.9, depthWrite: false, alphaTest: 0.32, cache: false }), l.length);
...
function Ae(e, t, o, n) {
  ...
  const s = new a.CanvasTexture(r);
  return s.colorSpace = a.SRGBColorSpace, s.anisotropy = 4, Oe.set(e, s), s;
}
const fn = "'Segoe UI', 'Noto Sans', system-ui, sans-serif", as = `'Noto Sans Telugu', 'Gautami', ${fn}`;
function Kt(e, t, o, n, r, s, c, i = 700, l = fn) {
  let f = Math.min(s, 100);
...
function ss(e, { bg: t = "#1d4e9c", fg: o = "#ffffff", name: n, telugu: r = "", strip: s = null }) {
  return Ae("fascia:" + e, 512, 128, (c, i, l) => {
    c.fillStyle = t, c.fillRect(0, 0, i, l), c.strokeStyle = "rgba(0,0,0,.35)", c.lineWidth = 6, c.strokeRect(4, 4, i - 8, l - 8), r ? (Kt(c, r, i / 2, l * 0.28, i - 40, 40, o, 700, as), Kt(c, n, i / 2, l * 0.7, i - 40, 44, o, 800)) : Kt(c, n, i / 2, l * 0.5, i - 40, 52, o, 800), s && (c.fillStyle = s, c.fillRect(0, l - 10, i, 10)), c.fillStyle = "rgba(255,255,255,.05)", c.fillRect(i * 0.12, l * 0.1,
...
function ls(e, t, o, n) {
  const r = Ae(e, t, o, n);
  return r.wrapS = r.wrapT = a.RepeatWrapping, r;
}
function ds(e) {
  const t = new a.Group();
  t.name = "mainRoad", e.add(t);
  const o = b({ color: g.road, bands: 3, tint: 7036528, flat: false }), n = b({ color: g.sidewalk, bands: 3, tint: 8022642, flat: false }), r = b({ color: g.sidewalkAlt, bands: 3, tint: 8022642, flat: false }), s = b({ color: g.curb, bands: 2, tint: 7036528 }), c = b({ color: 13218452, bands: 3, tint: 9072478, flat: false }), i = 0.012;
  for (const [l, f] of [[_t, -It], [It, Nt]]) {
    const u = Xt({ z0: l, z1: f, step: 1.6, a: (h) => ({ x: U(h) - F, y: i }), b: (h) => ({ x: U(h) + F, y: i }) }), d = new a.Mesh(u, o);
    d.receiveShadow = true, t.add(d);
  }
  {
  ...
  for (const l of [-1, 1]) for (const [f, u] of [[_t, -It], [It, Nt]]) {
    const d = (A) => U(A) + l * F, h = (A) => U(A) + l * (F + K), p = Xt({ z0: f, z1: u, step: 1.6, a: (A) => ({ x: d(A), y: Lt }), b: (A) => ({ x: h(A), y: Lt }) }), m = new a.Mesh(p, l < 0 ? n : r);
    m.receiveShadow = true, t.add(m);
    const w = Xt({ z0: f, z1: u, step: 1.6, a: (A) => ({ x: d(A), y: 0 }), b: (A) => ({ x: d(A), y: Lt }), flip: l > 0 }), M = new a.Mesh(w, s);
    t.add(M);
    const G = Xt({ z0: f, z1: u, step: 1.6, a: (A) => ({ x: h(A), y: -0.02 }), b: (A) => ({ x: h(A), y: Lt }), flip: l < 0 });
    t.add(new a.Mesh(G, s));
  }
  for (const l of [-1, 1]) {
    const f = Xt({ z0: _t, z1: Nt, step: 2, a: (d) => ({ x: U(d) + l * (F + K), y: 4e-3 }), b: (d) => ({ x: U(d) + l * (F + K + 3.4), y: 4e-3 }) }), u = new a.Mesh(f, c);
...
const Ke = F + K + 0.55, _ = {};
function un() {
  return _.done || (_.done = true, _.walls = [15983816, 15258542, 14673106, 15784128, 15129796, 14213348, 15653304, 14995392].map((e) => b({ color: e, bands: 3, tint: 9072480 })), _.trim = b({ color: 11573888, bands: 2, tint: 7035472 }), _.roof = b({ color: 12101776, bands: 3, tint: 7035472 }), _.roofDark = b({ color: 9075302, bands: 3, tint: 5983298 }), _.door = b({ color: 6047282, bands: 2, tint: 3812904 }),
  _.shutter = b({ color: 8226964, bands: 3, tint: 5265003 }), _.glass = I({ color: 4872816 }), _.grill = b({ color: 4934482, bands: 2, tint: 3816010 }), _.tank = b({ color: 3026483, bands: 3, tint: 3816010 }), _.awningA = b({ color: 13126460, bands: 3, tint: 8010298 }), _.awningB = b({ color: 3112299, bands: 3, tint: 2771530 }), _.ac = b({ color: 14210508, bands: 2, tint: 9079446 })), _;
}
function ko(e, t) {
  const o = t.side || 1;
  un();
  const n = new a.Group(), { x: r, z: s, w: c, d: i, h: l, sign: f } = t, u = _.walls[t.wall % _.walls.length], d = new a.Mesh(new a.BoxGeometry(i, l, c), u);
  d.position.set(r - o * i / 2, l / 2, s), n.add(d);
  const h = new a.Mesh(new a.BoxGeometry(i + 0.15, 0.42, c + 0.15), _.trim);
  h.position.set(r - o * i / 2, l + 0.18, s), n.add(h);
  const p = new a.Mesh(new a.CylinderGeometry(0.55, 0.55, 0.9, 12), _.tank);
  p.position.set(r - o * (i / 2 - 0.6), l + 0.85, s - c * 0.22), n.add(p);
  const m = new a.Mesh(new a.SphereGeometry(0.34, 10, 6, 0, Math.PI * 2, 0, Math.PI / 2), _.ac);
  if (m.rotation.x = Math.PI / 3, m.position.set(r - o * (i / 2 + 0.8), l + 0.55, s + c * 0.25), n.add(m), f) {
    ...
    P.position.set(r + o * 0.06, l - 1.45, s), n.add(P);
  }
  const w = c * 0.44, M = new a.Mesh(new a.BoxGeometry(0.08, 2.3, w), _.shutter);
  M.position.set(r + o * 0.02, 1.15, s - c * 0.18), n.add(M);
  for (let y = 0; y < 6; y++) {
    const k = new a.Mesh(new a.BoxGeometry(0.03, 0.035, w), _.grill);
    k.position.set(r + o * 0.07, 0.45 + y * 0.36, s - c * 0.18), n.add(k);
  }
  const G = new a.Mesh(new a.BoxGeometry(0.08, 2.1, c * 0.26), _.door);
  if (G.position.set(r + o * 0.02, 1.05, s + c * 0.24), n.add(G), t.awning) {
    const y = new a.Mesh(new a.BoxGeometry(1.5, 0.06, c * 0.8), t.awning === "a" ? _.awningA : _.awningB);
    y.rotation.z = -0.28 * o, y.position.set(r + o * 0.72, 2.62, s), n.add(y);
  }
  for (const y of [-c * 0.22, c * 0.22]) {
    const k = new a.Mesh(new a.BoxGeometry(0.06, 1.15, 0.95), _.glass);
    k.position.set(r + o * 0.03, l - 2.6, s + y), n.add(k);
    for (let P = -2; P <= 2; P++) {
      const v = new a.Mesh(new a.BoxGeometry(0.02, 1.15, 0.035), _.grill);
      v.position.set(r + o * 0.06, l - 2.6, s + y + P * 0.19), n.add(v);
    }
  }
  const A = new a.Mesh(new a.BoxGeometry(0.32, 0.5, 0.7), _.ac);
  A.position.set(r + o * 0.18, l - 0.65, s - c * 0.34), n.add(A), At(n), e.add(n);
  const x = o > 0 ? r - i : r, S = o > 0 ? r : r + i;
  return e.collide(x, s - c / 2, S, s + c / 2, l), n;
}
function fs(e, t) {
  un();
  const o = new a.Group(), { x: n, z: r, w: s, d: c, h: i } = t, l = _.walls[t.wall % _.walls.length], f = new a.Mesh(new a.BoxGeometry(c, i, s), l);
  if (f.position.set(n - c / 2, i / 2, r), o.add(f), t.roof === "slope") {
    const d = new a.Mesh(new a.CylinderGeometry(0.02, s * 0.62, 1.15, 4, 1), _.roofDark);
    d.rotation.y = Math.PI / 4, d.scale.z = c / (s * 0.62) * 0.5, d.position.set(n - c / 2, i + 0.56, r), o.add(d);
  } else {
    const d = new a.Mesh(new a.BoxGeometry(c + 0.14, 0.36, s + 0.14), _.trim);
    d.position.set(n - c / 2, i + 0.15, r), o.add(d);
    const h = new a.Mesh(new a.CylinderGeometry(0.5, 0.5, 0.8, 10), _.tank);
    h.position.set(n - c / 2 + 0.5, i + 0.75, r - s * 0.2), o.add(h);
  }
  const u = new a.Mesh(new a.BoxGeometry(0.07, 1.9, 0.9), _.door);
  u.position.set(n + 0.035, 0.95, r + s * 0.18), o.add(u);
  for (const d of [-s * 0.22]) {
    ...
    for (let p = -2; p <= 2; p++) {
      ...
      m.position.set(n + 0.055, 1.55, r + d + p * 0.18), o.add(m);
    }
  }
  return At(o), e.add(o), e.collide(n - c, r - s / 2, n, r + s / 2, i), o;
}
...
function hs(e) {
  let t = 6;
  us.forEach((r, s) => {
    ko(e, { x: U(t + 5 / 2) + Ke, z: t + 5 / 2, w: 5, d: 7.2, h: s % 3 === 2 ? 4.6 : 6.4, wall: s, sign: r, awning: s % 2 === 0 ? "a" : s % 3 === 0 ? "b" : null, side: -1 }), t += 5 + 0.15;
  });
  ...
  let n = 42;
  o.forEach((r, s) => {
    ko(e, { x: U(n + 5 / 2) - Ke - 3.6, z: n + 5 / 2, w: 5, d: 7.2, h: 4.6, wall: s + 2, sign: r, awning: s === 0 ? "b" : null, side: 1 }), n += 5 + 0.15;
  });
}
function ps(e) {
...
const So = F + K + 0.65, ce = 13, le = 41, J = 27, vt = 1.5;
function ms() {
...
function xs() {
  ...
  const o = (r, s, c = 3.2) => {
    t.beginPath(), t.arc(r, s, c, 0, 7), t.fill();
  };
  for (let r = 0; r < 9; r++) for (let s = 0; s < 9; s++) o(28 + r * 25 + (s % 2 ? 12 : 0), 28 + s * 25);
...
function ws(e) {
  ...
  const o = U(J), n = o - So, r = o - So - 17, s = b({ color: 15327172, bands: 3, tint: 9077362 }), c = b({ color: 13194015, bands: 3, tint: 7027252 }), i = b({ color: 14180906, bands: 3, tint: 8010272 }), l = b({ color: 14715482, bands: 3, tint: 9062960 }), f = b({ color: 14198844, bands: 3, tint: 9071146, emissive: 6901268, emissiveIntensity: 0.25 }), u = I({ color: 1511435 }), d = b({ color: 11911876,
  bands: 3, tint: 6978172 }), h = 2.05, p = [{ x0: n, z0: ce, x1: n + 0.35, z1: J - vt }, { x0: n, z0: J + vt, x1: n + 0.35, z1: le }, { x0: r, z0: ce, x1: r + 0.35, z1: le }, { x0: r, z0: ce, x1: n + 0.35, z1: ce + 0.35 }, { x0: r, z0: le - 0.35, x1: n + 0.35, z1: le }];
  for (const v of p) {
    const z = Math.max(v.x1 - v.x0, 0.35), R = Math.max(v.z1 - v.z0, 0.35), O = new a.Mesh(new a.BoxGeometry(z, h, R), s);
  ...
  y.position.set(r + 6.5, 2.2, J + 1.5), t.add(y);
  const k = new a.Mesh(new a.BoxGeometry(8.7, 0.3, 10.2), c);
  k.position.set(r + 6.5, 4.35, J + 1.5), t.add(k);
  for (const [v, z, R] of [[r + 2.5, ce + 3.5, 2.6], [n - 6.5, le - 2.2, 2.1]]) {
    const O = new a.Mesh(new a.CylinderGeometry(0.22, 0.3, 3.4, 8), b({ color: 5916210, bands: 3, tint: 3812898 }));
    O.position.set(v, 1.7, z), t.add(O);
    const H = new a.Mesh(new a.IcosahedronGeometry(R, 1), b({ color: 4156229, bands: 3, tint: 2771506 }));
...
const at = {};
function hn() {
  return at.pole || (at.pole = b({ color: 14078680, bands: 3, tint: 6972040 }), at.metal = b({ color: g.metal, bands: 3, tint: 6709392 }), at.metalDark = b({ color: g.metalDark, bands: 3, tint: 6051456 }), at.dark = b({ color: g.black, bands: 2, tint: 4932960 }), at.wire = b({ color: 4998744, bands: 2, tint: 4275288 }), at.red = b({ color: g.red, bands: 3, tint: 8011872 }), at.white = b({ color: g.
  wallWhite, bands: 3, tint: 7301008 }), at.concrete = b({ color: g.concrete, bands: 3, tint: 7301008 }), at.concreteMid = b({ color: g.concreteMid, bands: 3, tint: 6972040 }), at.terracotta = b({ color: 12941914, bands: 3, tint: 7296640 }), at.leaf = b({ color: g.leaf, bands: 3, tint: 5992332 }), at.leafDeep = b({ color: g.leafDeep, bands: 3, tint: 5992332 })), at;
}
function ys(e = {}) {
  const t = hn(), o = nt(e.seed ?? 5), n = new a.Group(), r = e.h ?? 9.2, s = { pole: [], metal: [], dark: [], white: [] }, c = (d, h, p) => s[d].push({ geometry: h, matrix: p });
  c("pole", new a.CylinderGeometry(0.11, 0.19, r, 8), T(0, r / 2, 0)), c("pole", new a.CylinderGeometry(0.24, 0.28, 0.22, 8), T(0, 0.11, 0));
  ...
  c("dark", new a.CylinderGeometry(0.045, 0.045, r - 1.4, 5), T(l * 0.135, (r - 1.4) / 2, 0.06));
...
function bs(e, t) {
  ...
  const r = n.length === 1 ? n[0] : pt(n.map((c) => ({ geometry: c }))), s = new a.Mesh(r, o.wire);
  return s.name = "wires", s.material = o.wire, e.add(s), n.forEach((c) => c !== r && c.dispose()), s;
}
...
function Ms(e) {
  ...
  for (const c of n) {
    ...
    const i = U(c) + r * (F + K - 0.28), l = ys({ seed: c * 31 | 0, h: 8.6, armDir: -r });
    l.position.set(i, e.groundAt(i, c), c), e.add(l), At(l), e.collide(i - 0.22, c - 0.22, i + 0.22, c + 0.22, 8.6), o.push({ x: i, z: c, side: r }), r = -r;
  }
  const s = [];
  for (const c of [-1, 1]) {
    ...
    for (const l of [0, -0.9]) {
      ...
      f.length > 1 && s.push({ points: f, sag: 0.55 });
    }
  }
  for (const c of [10, 26, 44, -16, -34]) {
    const i = U(c);
    s.push({ points: [new a.Vector3(i - (F + K - 0.28), 8.1, c), new a.Vector3(i + (F + K + 2.6), 6.4, c + 0.8)], sag: 0.7 });
  }
  s.push({ points: [new a.Vector3(U(-24) - 4.4, 7.9, -24), new a.Vector3(U(-6) - 4.5, 4.6, -6)], sag: 0.9 }), bs(e, s);
}
function zs(e) {
  const t = new a.Group(), o = -14, n = U(o) + F + K - 0.2, r = b({ color: 4877964, bands: 3, tint: 3820126 }), s = b({ color: 3626606, bands: 3, tint: 3029582 });
  for (const f of [-1.6, 1.6]) {
    const u = new a.Mesh(new a.CylinderGeometry(0.06, 0.06, 2.5, 8), r);
    u.position.set(n, 1.25, o + f), t.add(u);
  }
  const c = new a.Mesh(new a.BoxGeometry(1.7, 0.08, 4), s);
  c.rotation.z = -0.06, c.position.set(n - 0.3, 2.52, o), t.add(c);
  const i = new a.Mesh(new a.BoxGeometry(0.45, 0.08, 3.2), b({ color: 9071176, bands: 2, tint: 5916214 }));
  i.position.set(n + 0.35, 0.55, o), t.add(i);
  for (const f of [-1.3, 1.3]) {
    const u = new a.Mesh(new a.BoxGeometry(0.4, 0.5, 0.08), r);
...
function vs(e) {
  const t = b({ color: 7039858, bands: 2, tint: 3816010 });
  for (const { z: o, side: n } of [{ z: 45, side: -1 }, { z: -20, side: 1 }]) {
    const r = U(o) + n * (F + K + 0.15), s = new a.Mesh(new a.CylinderGeometry(0.05, 0.05, 2.9, 8), t);
    s.position.set(r, e.groundAt(r, o) + 1.45, o), e.add(s);
...
function ks(e, t = 1) {
  const o = new a.Group(), n = b({ color: 7230272, bands: 3, tint: 4864560 }), r = [5214047, 6265940, 4161359], s = 2.6 * t, c = new a.Mesh(new a.CylinderGeometry(0.14 * t, 0.24 * t, s, 7), n);
  c.position.y = s / 2, o.add(c);
  const i = e.int(3, 5);
  for (let l = 0; l < i; l++) {
    const f = e.range(0.9, 1.5) * t, u = new a.Mesh(new a.IcosahedronGeometry(f, 1), b({ color: e.pick(r), bands: 3, tint: 3825482 }));
    u.position.set(e.range(-0.8, 0.8) * t, s + e.range(-0.2, 0.9) * t, e.range(-0.8, 0.8) * t), u.scale.y = 0.72, o.add(u);
  }
  return At(o), o;
}
function Ss(e) {
  const t = nt(777), o = [{ x: -10.5, z: 20, s: 1.5, c: true }, { x: -13.5, z: 34, s: 1.35, c: true }, { x: -5.85, z: 22.5, s: 1.25, c: true }, { x: -5.7, z: -14, s: 1.1, c: true }, { x: F + K + 1.1, z: -27, s: 1, c: true }, { x: -5.9, z: -44, s: 1.2, c: true }, { x: F + K + 1, z: -58, s: 1, c: true }, { x: F + K + 1.2, z: 50, s: 1.1, c: true }, { x: -5.7, z: 52.5, s: 0.95, c: true }, { x: F + K +
  1.3, z: 66, s: 1.15, c: true }];
  for (const s of o) {
    const c = U(s.z) + s.x, i = ks(t, s.s);
    i.position.set(c, e.groundAt(c, s.z), s.z), e.add(i), s.c && e.collide(c - 0.26, s.z - 0.26, c + 0.26, s.z + 0.26, 2.4);
  }
  const n = new a.Group(), r = [14043531, 14708895, 12728957];
  for (let s = 0; s < 60; s++) {
    const c = 14.5 + s * 0.42 + t.range(-0.1, 0.1), i = U(c) - (F + K + 0.65) + t.range(-0.3, 0.3), l = t.range(0.13, 0.3), f = new a.Mesh(new a.IcosahedronGeometry(l, 1), b({ color: t.pick(r), bands: 3, tint: 7027294 }));
    f.position.set(i, 1.9 + t.range(-0.35, 0.45), c), n.add(f);
  }
  e.add(n);
}
function Gs(e) {
  const t = new a.Group(), o = b({ color: 9071170, bands: 3, tint: 4864548 }), n = b({ color: 10123850, bands: 3, tint: 5916208 }), r = [4160053, 14186274, 11743532, 5909867, 8034874, 14201402], s = [{ z: 31.5, tarp: 2775706 }, { z: 35.4, tarp: 3832394 }];
  for (const c of s) {
    const i = U(c.z) - (F + K - 1);
    ...
    const l = new a.Mesh(new a.BoxGeometry(2.1, 0.05, 2.6), b({ color: c.tarp, bands: 3, tint: 1714746 }));
    ...
    for (const h of [-0.6, 0, 0.6]) for (const p of [-0.4, 0.05, 0.5]) {
      const m = new a.Mesh(new a.IcosahedronGeometry(0.15, 1), b({ color: r[u % r.length], bands: 3, tint: 2767394 }));
...
const tt = {};
function Ts() {
  return tt.concrete || (tt.concrete = b({ color: g.concrete, bands: 3, tint: 7301008 }), tt.concreteMid = b({ color: g.concreteMid, bands: 3, tint: 6972040 }), tt.metal = b({ color: g.metal, bands: 3, tint: 6709392 }), tt.metalDark = b({ color: g.metalDark, bands: 3, tint: 6051456 }), tt.dark = b({ color: g.black, bands: 2, tint: 4932960 }), tt.shell = b({ color: 12896462, bands: 3, tint: 6709392 }),
  tt.shellTrim = b({ color: 10133672, bands: 3, tint: 6051456 }), tt.wood = b({ color: 10256222, bands: 3, tint: 6051456 }), tt.woodDark = b({ color: 8217416, bands: 3, tint: 6051456 }), tt.soil = b({ color: 7627342, bands: 3, tint: 6380160 }), tt.bamboo = b({ color: g.bamboo, bands: 3, flat: false, tint: 5992332 }), tt.twine = b({ color: g.rope, bands: 3, flat: false, tint: 7301008 }), tt.pale = I(
  { color: 16184040 })), tt;
}
...
function X(e, t, o, n, r = 6) {
  const s = new a.Vector3().subVectors(o, t), c = s.length();
  c < 1e-4 || e.push({ geometry: Cs(r), matrix: new a.Matrix4().compose(new a.Vector3().addVectors(t, o).multiplyScalar(0.5), new a.Quaternion().setFromUnitVectors(As, s.normalize()), N(n, c, n)) });
}
function Rs(e, t, o, n = {}) {
  const r = n.noCast ?? [];
  for (const s of Object.keys(t)) {
    if (!t[s].length) continue;
    const c = new a.Mesh(pt(t[s]), o[s]);
    c.castShadow = !r.includes(s), c.receiveShadow = true, e.add(c), s === n.outline && wt(c, { thickness: n.thickness ?? 32e-4 });
  }
  return e;
}
function Go(e = {}) {
  ...
  o.add(n);
  const r = 0.2, s = { RA: N(-0.6, r), FA: N(0.57, r), ENG: N(-0.3, 0.28), RS: N(-0.22, 0.3), FS: N(0.3, 0.28), HS: N(0.48, 0.62), HT: N(0.4, 0.96), BAR: N(0.38, 1), SHK: N(-0.44, 0.5) }, c = { dark: [], metal: [], body: [], amber: [], dial: [] }, i = (f, u, d) => c[f].push({ geometry: u, matrix: d }), l = b({ color: e.color ?? 13227228, bands: 3, tint: 7301008 });
  for (const f of [s.RA, s.FA]) i("dark", new a.CylinderGeometry(r, r, 0.09, 14), T(f.x, f.y, 0, Math.PI / 2)), i("metal", new a.CylinderGeometry(0.125, 0.125, 0.11, 12), T(f.x, f.y, 0, Math.PI / 2)), i("dark", new a.CylinderGeometry(0.038, 0.038, 0.13, 8), T(f.x, f.y, 0, Math.PI / 2));
  i("body", new a.TorusGeometry(r + 0.04, 0.026, 4, 14, Math.PI * 0.85), T(s.FA.x, s.FA.y, 0, 0, 0, -0.72, 1, 1, 2.2)), i("body", new a.TorusGeometry(r + 0.05, 0.032, 4, 12, Math.PI * 0.62), T(s.RA.x, s.RA.y, 0, 0, 0, 0.55, 1, 1, 1.9));
  for (const f of [-1, 1]) X(c.metal, N(s.HS.x, s.HS.y, f * 0.055), N(s.FA.x, s.FA.y, f * 0.055), 0.019), X(c.metal, N(s.ENG.x, s.ENG.y, f * 0.062), N(s.RA.x, s.RA.y, f * 0.062), 0.022);
  X(c.metal, s.HS, s.HT, 0.026), X(c.metal, s.HS, s.FS, 0.026), X(c.metal, s.FS, s.RS, 0.024), X(c.metal, s.RS, s.SHK, 0.024), X(c.metal, N(s.SHK.x, s.SHK.y, 0.07), N(s.RA.x + 0.02, s.RA.y + 0.04, 0.07), 0.024), X(c.metal, N(-0.14, 0.26, -0.09), N(-0.24, 0.015, -0.19), 0.016);
  ...
  {
    const f = I({ color: g.wallGray }), u = I({ color: 16777215, map: En(), cache: false }), d = new a.Mesh(new a.BoxGeometry(0.02, 0.13, 0.24), [f, u, f, f, f, f]);
    d.position.set(-0.77, 0.42, 0), d.castShadow = true, n.add(d);
  }
  i("metal", new a.CylinderGeometry(0.018, 0.018, 0.56, 6), T(s.BAR.x, s.BAR.y, 0, Math.PI / 2)), X(c.metal, s.HT, s.BAR, 0.022);
  for (const f of [-1, 1]) i("dark", new a.CylinderGeometry(0.024, 0.024, 0.11, 6), T(s.BAR.x, s.BAR.y, f * 0.22, Math.PI / 2)), X(c.metal, N(0.36, 1.02, f * 0.18), N(0.32, 1.24, f * 0.24), 0.012), i("dark", new a.BoxGeometry(0.03, 0.11, 0.14), T(0.31, 1.26, f * 0.25)), n.add(D(8e-3, 0.09, 0.12, I({ color: g.mirrorFace }), 0.294, 1.26, f * 0.25)), i("amber", new a.BoxGeometry(0.06, 0.05, 0.05), T(
  0.45, 0.86, f * 0.19));
  ...
  return Rs(n, c, { dark: t.dark, metal: t.metal, body: l, amber: b({ color: g.orange, bands: 2, tint: 9396304 }), dial: I({ color: 15328986 }) }, { outline: "body", thickness: 34e-4 }), n.rotation.x = e.lean ?? -0.09, o.position.set(e.x, e.y ?? 0, e.z), o.rotation.y = e.ry ?? 0, o.userData.inner = n, o;
}
function qe(e = {}) {
  const t = new a.Group(), o = new a.Group();
  t.add(o), t.userData.inner = o;
  const n = b({ color: e.color ?? 15251488, bands: 3, tint: 9071146 }), r = b({ color: 2302758, bands: 2, tint: 3816010 }), s = b({ color: 3026483, bands: 2, tint: 3816010 }), c = b({ color: 10133672, bands: 3, tint: 6051456 }), i = I({ color: 10338516, transparent: true, opacity: 0.55 }), l = { yellow: [], black: [], dark: [], metal: [] }, f = (p, m, w) => l[p].push({ geometry: m, matrix: w }), u = 0.25;
  ...
  const h = { yellow: n, black: r, dark: s, metal: c };
...
function $e(e, t, o, n, r) {
  te(t, o, To, Ao, Co), Ro.makeBasis(Ao, To, Co), Po.setFromRotationMatrix(Ro), Eo.set(0, r + Math.PI / 2, 0, "YXZ"), Bo.setFromEuler(Eo), e.quaternion.copy(Po).multiply(Bo), Yt(t, n, o, e.position);
}
function Ps(e) {
  ...
  for (const i of n) {
    const l = U(i.z) + i.x * (F + 0.75), f = i.kind === "auto" ? qe({ color: i.color, lean: -0.03 }) : Go({ color: i.color ?? 13227228 });
  ...
  function c(i) {
    for (const l of t) {
      ...
      if (r()) {
        ...
        (l.dir > 0 ? l.z < -s && h >= -s : l.z > s && h <= s) && (f = 0), l.dir > 0 && l.z < -s - 0.01 && h > -s && (f = 0), l.dir < 0 && l.z > s + 0.01 && h < s && (f = 0);
      }
...
function Io(e) {
  const t = new a.Group(), o = b({ color: e.pick(Bs), bands: 3, tint: 9072478 }), n = b({ color: e.pick(Es), bands: 3, tint: 7036528 }), r = b({ color: e.pick(Is), bands: 2, tint: 3816010 }), s = b({ color: 2367518, bands: 2, tint: 3816010 }), c = e.range(0.92, 1.06), i = new a.Mesh(new a.BoxGeometry(0.34, 0.55, 0.2), n);
  i.position.y = 1.06 * c, t.add(i);
  const l = new a.Mesh(new a.SphereGeometry(0.115, 10, 8), o);
  l.position.y = 1.5 * c, t.add(l);
  const f = new a.Mesh(new a.SphereGeometry(0.118, 10, 8, 0, Math.PI * 2, 0, Math.PI / 2.2), s);
  f.position.y = 1.51 * c, t.add(f);
  const u = new a.Mesh(new a.BoxGeometry(0.11, 0.78, 0.13), r), d = u.clone();
...
function _s(e) {
  for (const t of Ls) {
    const o = U(t.pos(0).z), n = t.pos(o), r = new a.Mesh(new a.BoxGeometry(1.8, 2.4, 2.6), I({ color: 16711680, cache: false }));
...
function Os(e) {
  ...
  const o = [], n = [], r = [], s = [], c = [], i = { scene: e, root: t, colliders: o, interactables: n, add: (x) => (t.add(x), x), collide: (x, S, y, k, P, v) => {
    o.push({ x0: Math.min(x, y), x1: Math.max(x, y), z0: Math.min(S, k), z1: Math.max(S, k), top: P, bottom: v });
  }, platform: (x) => s.push(x), cut: (x) => c.push(x), groundAt: (x, S) => {
    let y = io(x, S);
    for (const k of c) x > k.x0 && x < k.x1 && S > k.z0 && S < k.z1 && (y = Math.min(y, k.top));
    for (const k of s) x > k.x0 && x < k.x1 && S > k.z0 && S < k.z1 && (y = Math.max(y, k.top));
    return y;
  }, interact: (x) => n.push(x), update: (x) => r.push(x) }, l = Yn(e), f = Ka(i), u = es(i), d = Ns(i), h = os(i), p = 165, m = { blink: 0, armT: 0 };
  f.request = () => {
    u.x = gt(-153 * u.dir);
  };
  const w = [1, -1].map((x) => {
    const S = { x0: U(0) - F - 0.6, x1: U(0) + F + 0.6, z0: x * ke - 0.16, z1: x * ke + 0.16, top: -1 };
    return o.push(S), S;
  });
  function M(x) {
  ...
  return u.planetize(), { root: t, colliders: o, platforms: s, cuts: c, interactables: n, train: u, crossing: f, planet: l, petals: h, bakeStats: G, bounds: { z0: -ht * 0.24, z1: ht * 0.24 }, heightAt(x, S, y) {
    let k = io(x, S);
    for (const v of c) x > v.x0 && x < v.x1 && S > v.z0 && S < v.z1 && (k = Math.min(k, v.top));
    const P = y === void 0 ? 1 / 0 : y + 0.55;
    for (const v of s) v.top > P || x > v.x0 && x < v.x1 && S > v.z0 && S < v.z1 && (k = Math.max(k, v.top));
    return k;
  }, update(x) {
    M(x), u.update(x);
    for (const S of r) S(x);
...
function Hs({ scene: e, world: t, player: o, hud: n }) {
  const r = qe({ color: 15251488, lean: No }), s = r.userData.inner;
  ...
  function G(C, L, V, B, E) {
    te(C, L, u, d, h), p.makeBasis(d, u, h), m.setFromRotationMatrix(p), M.set(0, B + Math.PI / 2, E, "YXZ"), w.setFromEuler(M), r.quaternion.copy(m).multiply(w), Yt(C, V, L, r.position), r.updateMatrixWorld(true);
  }
  function A(C, L, V, B) {
    const E = -Math.sin(V), j = -Math.cos(V), lt = _o / 2, Ce = t.heightAt(gt(C + E * lt), L + j * lt, B), oe = t.heightAt(gt(C - E * lt), L - j * lt, B);
  ...
  function x(C) {
    ...
    if (!C) return;
    const L = Math.abs(Math.sin(l.heading)), V = Math.abs(Math.cos(l.heading)), B = Do * L + Lo * V, E = Do * V + Lo * L, j = t.heightAt(l.x, l.z, o.pos.y);
    f = { x0: l.x - B, x1: l.x + B, z0: l.z - E, z1: l.z + E, top: j + 1.02 }, t.colliders.push(f);
  }
  function S(C, L, V, B) {
    for (const E of t.colliders) if (E !== f && !(E.top !== void 0 && E.top <= B + Ws) && !(E.bottom !== void 0 && E.bottom > B + 1.9) && C > E.x0 - V && C < E.x1 + V && L > E.z0 - V && L < E.z1 + V) return false;
    return true;
  }
  function y(C) {
  ...
  function k() {
    ...
    s.rotation.x = No, G(l.x, l.z, C, l.heading, A(l.x, l.z, l.heading, C)), r.visible = true, l.out = true, x(true), y(true);
  }
  function P() {
    const C = o.pos.y;
    let L = null;
    for (const V of [2, 1.65, 2.6, 1.3]) {
      for (const B of [0, 0.45, -0.45, 0.95, -0.95, 1.6, -1.6]) {
        const E = gt(o.pos.x - Math.sin(o.yaw + B) * V), j = o.pos.z - Math.cos(o.yaw + B) * V;
  ...
  function v() {
    l.riding || (r.visible = false, l.out = false, x(false), y(false), n?.flash("auto  \xB7  put away", 1200));
  }
  function z() {
  ...
  function R() {
    ...
    const C = Math.cos(l.heading), L = -Math.sin(l.heading), V = -Math.sin(l.heading), B = -Math.cos(l.heading), E = o.pos.y, j = [[-C * 1.35, -L * 1.35], [C * 1.35, L * 1.35], [-V * 1.9, -B * 1.9]];
  ...
  function H() {
    if (!l.riding) return;
    const C = o.yaw, L = -Math.sin(C), V = -Math.cos(C), B = gt(o.pos.x + L * Ot.seatFwd), E = o.pos.z + V * Ot.seatFwd;
    G(B, E, o.pos.y, C, A(B, E, C, o.pos.y)), s.rotation.x = -o.roll * Fs, l.x = B, l.z = E, l.heading = C;
  }
  return { group: r, toggle: O, summon: P, recall: v, mount: z, dismount: R, update: H, get riding() {
    return l.riding;
  }, get summoned() {
    return l.out;
  } };
...
Ht.setClearColor(new a.Color(g.fog), 1);
const it = new a.Scene();
it.fog = new a.Fog(g.fog, 44, 205);
const ct = new a.PerspectiveCamera(46, 1, 0.25, 600);
ct.rotation.order = "YXZ";
const Z = new a.DirectionalLight(g.sun, 2.25);
...
it.add(Z);
it.add(Z.target);
const ee = new a.DirectionalLight(g.fill, 1.08);
...
it.add(qt);
it.add(qt.target);
const Ge = new a.HemisphereLight(g.hemiSky, g.hemiGround, 1.12);
it.add(Ge);
